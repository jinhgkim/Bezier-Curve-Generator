...
function init_event_handlers (canvas, curve, gl_operation) {
    var lastX = -1;
    var lastY = -1;
    ...
    canvas.onmousedown = function(ev) {  //Mouse is pressed
        ...
        if(rect.left <= x && x <= rect.right &&
           rect.top <= y && y <= rect.bottom) {
            ...
            chosen_point = if_click_point(point, curve.get_control_points());
    ...
    canvas.onmousemove = function(ev) { //Mouse is moved
        if (dragging) {
            ...
            var point = curve.get_control_point(chosen_point);
            point.set(x, y);
...
curve.set_GL(gl_operation);

draw(curve);


// Initial canvas event handlers
init_event_handlers(canvas, curve, gl_operation);
...
subdivisionInput.addEventListener("focusout", function(){
    ...
    curve.set_subdivision_level(input_value);
    draw(curve);
});
...
piecewiseInput.addEventListener("focusout", function(){
    ...
    curve.set_piecewise_degree(input_value);
    draw(curve);
});
